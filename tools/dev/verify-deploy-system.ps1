...
function Deploy-Args([string[]]$extra, [string]$proc = 'McpLinkNoSuchProcess') {
    return @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $scriptUnderTest,
             '-Dst', $dst, '-DstHotReload', $dhr, '-Cfg', $cfg, '-StageDir', $stage,
             '-Log', $log, '-RetrySeconds', '5', '-WaiterLaunch', 'none',
             '-GameProcessName', $proc) + $extra
}
function Invoke-DeployScript([string[]]$extra, [string]$proc = 'McpLinkNoSuchProcess') {
    return Start-Process powershell -ArgumentList (Quote-Args (Deploy-Args $extra $proc)) -PassThru -Wait -WindowStyle Hidden
}
function Start-WaiterProc {
    return Start-Process powershell -ArgumentList ('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $scriptUnderTest, '-Waiter', '-StageDir', $stage, '-Log', $log) -PassThru -WindowStyle Hidden
}
