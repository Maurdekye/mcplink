...
// Reported in-world 2026-08-24: the superior→subordinate wire left the superior's bottom edge
// pointing down and ALSO entered the subordinate's top edge pointing down (it should arrive from
// above), and its texture showed all five atlas styles crushed across the wire's thin width.
//
// Neither is checkable by looking at a rendered frame from here, so both are pinned against the
// ENGINE'S OWN definitions instead: the curve below is WireMeshBase.UpdateMeshData transcribed
// literally, and the atlas arithmetic is ProtoFluxWireManager.Setup's, cross-checked against
// DatatypeColorHelper.GetWireAtlasOffset. Appearance stays the user's call after deploy.

// WireMeshBase.UpdateMeshData, transcribed: the curve point at parameter t. Note the shape —
// lerp(P0 + T0·t, P1 + T1·(1−t)) — which is why BOTH tangents are handles pointing OUT of their
// own endpoint, and why Tangent1 is not a direction of travel.
static float3 WireCurve(float3 p0, float3 p1, float3 t0, float3 t1, float t)
{
...
// Two upright panels, superior above-left of its subordinate. Unrotated, so a panel-local axis
// IS its world axis and LocalDirectionToGlobal drops out of the arithmetic.
