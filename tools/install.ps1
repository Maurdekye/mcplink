...
try {
    ...
    Write-Host "  2. Connect Codex, Gemini CLI or Claude Code (recommended route, needs Python 3.8+):"
    Write-Host "       codex mcp add mcplink -- python `"$proxyPath`""
    Write-Host "       gemini mcp add mcplink python `"$proxyPath`""
    ...
    Write-Host "       gemini mcp add --transport http mcplink http://localhost:7357/mcp"
